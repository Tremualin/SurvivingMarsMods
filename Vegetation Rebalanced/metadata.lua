return PlaceObj('ModDef', {
	'title', "Vegetation Rebalanced",
	'description', "ForestationPlant will now receive up to a +400% boost based on current Atmosphere, Temperature, Water, and Vegetation levels\nForestation plant will not stop contributing to global vegetation once it reaches 40%\nVegetation spreads passively based on % squared (at 10% it will be 0.01%, at 50% it will be 0.25%) even without forestation plants\n The passive spread is also boosted by Atmosphere, Temperature and Water\nHowever, Dust Storm now also require 30% Vegetation in addition to 50% Atmosphere to stop happening\nNot compatible with Vegetation Goes to 11 by Choggi",
	'id', "Tremualin_Vegetation_Rebalanced",
	'pops_desktop_uuid', "6c2c7e13-d0a4-418d-baf9-784d03a83963",
	'pops_any_uuid', "50d68d9d-303f-479b-8866-f8be05694c4c",
	'author', "Tremualin",
	'image', "Preview.png",
	'version', 1,
	'lua_revision', 233360,
	'saved_with_revision', 1001586,
	'saved', 1621651364,
	"code", {
		"script.lua",
	},
})