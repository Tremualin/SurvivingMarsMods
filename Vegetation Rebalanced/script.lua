function ForestationPlant:GetTerraformingBoostSol()
	...
	local terraforming_progress_boost = GetTerraformParam("Atmosphere") + GetTerraformParam("Temperature") + GetTerraformParam("Water") + GetTerraformParam("Vegetation")
	...
	local weightedBoost = MulDivRound(self.terraforming_boost_sol, terraforming_progress_boost, 100*const.TerraformingScale)
...
function OnMsg.ClassesPostprocess()
	for _, preset in ipairs(Presets.TerraformingParam.Default or empty_table) do
		if preset.id == "Vegetation" then
	    	local newItem = PlaceObj("ThresholdItem", {"Id","DustStormStop","Threshold",30,"Hysteresis",2})
...
function GetVegetationBoom()
  -- Vegetation boosts itself: (vegetation%)^2, i.e: 10% would boost by 0.01% per sol
	local vegetationSquared = MulDivRound(GetTerraformParam("Vegetation"), GetTerraformParam("Vegetation"), 100*100*const.TerraformingScale)

  -- Then Water, Temperature and Atmosphere boost it even further; 50% atmosphere, water and temperature would boost it by +150%
  local terraforming_progress_boost = GetTerraformParam("Atmosphere") + GetTerraformParam("Temperature") + GetTerraformParam("Water")
	local weightedBoost = MulDivRound(vegetationSquared, terraforming_progress_boost, 100*const.TerraformingScale)
...
function OnMsg.LoadGame()
	...
	VegetationBoomThread = (CreateGameTimeThread(function()
	...
	for _, preset in ipairs(Presets.TerraformingParam.Default or empty_table) do
		if preset.id == "Vegetation" then
			...
			local newFactor = PlaceObj("TerraformingFactorItem", {
		      ...
		      "VegetationBoom",
		      "display_name",
		      "Wild vegetation boom",
		      "units",
		      "PerSol",
		      "GetFactorValue",
		      function(self)
		        return GetVegetationBoom()
		      end
		    })
