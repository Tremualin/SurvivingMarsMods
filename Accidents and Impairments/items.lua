    ...
    PlaceObj('ModItemTraitPreset', {
        ...
        function (colonist, trait)
            ...
            if not residence or residence.service_comfort < 65 * const.Scale.Stat then
                colonist:ChangeSanity(-5 * const.Scale.Stat, "My residence is inadequate for me (Sensory Impaired) ")
            end
            if not UIColony:IsTechResearched("SupportiveCommunity") then
                Tremualin.Functions.TemporarilyModifyMorale(colonist, -20, 0, 1, "Tremualin_SensoryImpaired", "This community isn't doesn't support people like me")
            end
        end,
