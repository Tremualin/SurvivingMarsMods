return {
    ...
    PlaceObj('ModItemTraitPreset', {
        ...
        function (colonist, trait)
            ...
            if colonist:GetHealth() > 70 then
                ...
                if #victims > 0 then
                    for i = 1, 3 do
                        local victim = table.rand(victims)
                        -- Only fight if the victim is also healthy - this could have changed since we last checked
                        if victim:GetHealth() > 70 then
        ...
        description = T(751189543735, --[[ModItemTraitPreset Brawler description]] "Once a day, if health > 70, this an another non-child with health > 70 will both lose 10 health; then both colonists regain 5 sanity. "),
    ...
    PlaceObj('ModItemTraitPreset', {
        ...
        function (colonist, trait)
            ...
            if colonist:Random(100) < 80 then
    ...
    PlaceObj('ModItemTraitPreset', {
        ...
        function (colonist, trait)
            ...
            if colonist_with_lowest_sanity then
                colonist_with_lowest_sanity:ChangeSanity(8 * const.Scale.Stat, "Someone listened to my woes ")
                colonist:ChangeSanity(-4 * const.Scale.Stat, "It ain't easy being a good listener ")
            end
        end,
        description = T(810886541611, --[[ModItemTraitPreset Listener description]] "The colonist with the lowest sanity in the residence gets +8 sanity. But this colonist will lose 4 sanity. "),
